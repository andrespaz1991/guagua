...
class Zebra_Pagination
{
    ...
    public function base_url($base_url = '', $preserve_query_string = true)
    {
        ...
        // set the base URL
    #    $base_url = ($base_url == '' ? $host= $_SERVER['REQUEST_URI'] : $base_url);
     #   $base_url=$host.$url;
...
#echo $parsed_url;
        ...
        $this->_properties['base_url'] = $base_url;
    ...
    public function get_page()
    {
        ...
        if (!$this->_properties['page_set']) {
             if ($this->_properties['method'] == 'cookie' and isset($_COOKIE[$this->_properties['variable_name']])){
                 $this->set_page((int)$_COOKIE[$this->_properties['variable_name']]);
             }elseif ($this->_properties['method'] == 'post' and isset($_POST[$this->_properties['variable_name']])){
                 $this->set_page((int)$_POST[$this->_properties['variable_name']]);
             }elseif(
    ...
    public function method($method = 'get')
    {
        ...
        }elseif(strtolower($method) == 'cookie'){
        $this->_properties['method'] = 'cookie';
        }else{
    ...
    {
        if ($this->_properties['method'] == 'cookie'){
        $this->render2($return_output);
        }elseif ($this->_properties['method'] == 'post'){
        $this->render3($return_output);
        }elseif ($this->_properties['method'] == 'url'){
        $this->render1($return_output);
        }elseif ($this->_properties['method'] == 'get'){
        $this->render1($return_output);
        }else{
        $this->render1($return_output);
        }
    }
    public function render1($return_output = false)
    {
        ...
        $output = '<div class="Zebra_Pagination"><ul class="pagination justify-content-center">';
    ...
    public function render3($return_output = false)
    {
        ...
        if ($this->_properties['reverse']) {
            ...
            if ($this->_properties['navigation_position'] == 'left')
                ...
                // first show next/previous and then page links
                $output .= $this->_show_next3() . $this->_show_previous3() . $this->_show_pages3();

            // if "next page" and "previous page" links are to be shown to the right of the links to individual pages
            elseif ($this->_properties['navigation_position'] == 'right')

                $output .= $this->_show_pages3() . $this->_show_next3() . $this->_show_previous3();

            // if "next page" and "previous page" links are to be shown on the outside of the links to individual pages
            else $output .= $this->_show_next3() . $this->_show_pages3() . $this->_show_previous3();
        ...
        // if we're showing records in natural order
            ...
            if ($this->_properties['navigation_position'] == 'left')
                ...
                // first show next/previous and then page links
                $output .= $this->_show_previous3() . $this->_show_next3() . $this->_show_pages3();

            // if "next page" and "previous page" links are to be shown to the right of the links to individual pages
            elseif ($this->_properties['navigation_position'] == 'right')

                $output .= $this->_show_pages3() . $this->_show_previous3() . $this->_show_next3();

            // if "next page" and "previous page" links are to be shown on the outside of the links to individual pages
            else $output .= $this->_show_previous3() . $this->_show_pages3() . $this->_show_next3();
    ...
    {
#echo $this->_properties['base_url'];
    ...
    private function _show_next()
    {
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            $output = '<li' . ($this->_properties['page'] == $this->_properties['total_pages'] ? ' class="disabled"' : '') . '><a href="' .             // the href is different if we're on the last page
...
$idform = uniqid("form_");
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            // if we're on the last page, the link is disabled
            $output = '<li' . ($this->_properties['page'] == $this->_properties['total_pages'] ? ' class="disabled"' : '') . '><a onclick="';
            $output .= $this->_properties['page'] == $this->_properties['total_pages'] ? 'javascript:void(0);' : 'document.getElementById(\''.$idform.'\').submit();';
            $output .= '" rel="next">';
                $output .= ($this->_properties['reverse'] ? $this->_properties['previous'] : $this->_properties['next']) . '</a>';
                $output .= '<form style="display:inline" method="post" id="'.$idform.'" action="'.($this->_properties['base_url']).'">';
                $output .= $this->repetir_post();
                 $output .= '<input type="hidden" name="'.$this->_properties['variable_name'].'" value="'.($this->_properties['page'] == $this->_properties['total_pages'] ? 1 : $this->_properties['page'] + 1).'">';
                $output .= '</form>';
                $output .= '</li>';
        // return the resulting string
        return $output;
     ...
     *  @access private
     */
    public function repetir_post(){
        /* inicio repetir los datos que llegan por POST*/
        $salida = '';
        if (!empty($_POST))
    ...
    private function _show_pages()
    {
        ...
        if ($this->_properties['total_pages'] <= $this->_properties['selectable_pages']) {
            ...
            for (
                ...
                $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '>';
        ...
        // if the total number of pages is greater than the number of selectable pages
            ...
            $output .= '<li' . ($this->_properties['page'] == ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) ? ' class="active"' : '') . '>';
            $output .= '<a href="' . $this->_build_uri($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) . '">' .
            ...
            for ($i = $starting_page; $this->_properties['reverse'] ? $i >= $ending_page : $i <= $ending_page; $this->_properties['reverse'] ? $i-- : $i++)
                ...
                $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '><a as href="' . $this->_build_uri($i) . '">' .($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), ' ', STR_PAD_LEFT) : $i) .
    ...
    private function _show_pages2()
    {
        ...
        // if the total number of pages is greater than the number of selectable pages
            ...
            // highlight if the page is currently selected
            $output .= '<li ';
            $output .= ($this->_properties['page'] == ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) ? ' class="active"' : '');
            $output .= '><a onclick="grabarcookie(\''.$this->_properties['cookie_page'].'\',\'';
            $output .= $this->_properties['reverse'] ? $this->_properties['total_pages'] : 1 . '\');'.$this->_properties['fn_js_page'].'">';
            // if padding is required
            $output .= ($this->_properties['padding'] ?
            ...
            // also, highlight if it is the currently selected page
            $output .= '<li '; 
            $output .= ($this->_properties['page'] == $i ? ' class="active"' : '');
            $output .= '><a onclick="grabarcookie(\''.$this->_properties['cookie_page'].'\',\'';
            $output .= $this->_properties['reverse'] ? 1 : $this->_properties['total_pages'] . '\');';
            $output .= $this->_properties['fn_js_page'].'">';

                // also, apply padding if necessary
            $output .= ($this->_properties['padding'] ? str_pad(($this->_properties['reverse'] ? 1 : $this->_properties['total_pages']), strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : ($this->_properties['reverse'] ? 1 : $this->_properties['total_pages'])) .
...
$idform = uniqid("form_");
                // render the link for each page making sure to highlight the currently selected page
                $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '>';
                    $output .= '<a onclick="document.getElementById(\''.$idform.'\').submit();">';
                    // apply padding if required
                    $output .= ($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : $i).'</a>';
                    $output .= '<form style="display:inline" method="post" id="'.$idform.'" action="' .($this->_properties['base_url']). '">';
                    $output .= $this->repetir_post();
                   $output .= '<input type="hidden" name="page" value="'.
                    ($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : $i).
...
$idform = uniqid("form_");
            ...
            $output .= '<li' . ($this->_properties['page'] == ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) ? ' class="active"' : '') . '>';
            $output .= '<a onclick="document.getElementById(\''.$idform.'\').submit();">';
            $output .= ($this->_properties['padding'] ? str_pad(($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1), strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1));
            $output .= '</a>';
            $output .='<form method="post" id="'.$idform.'" action="' .($this->_properties['base_url']). '">';
                    $output .= $this->repetir_post();
                    // apply padding if required
                    $output .= '<input type="submit" name="page" value="'.($this->_properties['padding'] ? str_pad(($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1), strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1)) .
            ...
            for ($i = $starting_page; $this->_properties['reverse'] ? $i >= $ending_page : $i <= $ending_page; $this->_properties['reverse'] ? $i-- : $i++){
                ...
                $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '>';
                 $output .= '<a onclick="document.getElementById(\''.$idform.'\').submit();">';
                 $output .= ($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : $i);
                 $output .= '</a>';
                    $output .= $this->repetir_post();
                 $output .= '<form style="display:inline" method="post" id="'.$idform.' action="' .($this->_properties['base_url']). '">';
                    // apply padding if required
                    $output .= '<input type="submit" name="page" value="'.($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : $i) .
            ...
            $output .= '<form style="display:inline" method="post" id="'.$idform.' action="' . ($this->_properties['base_url']) . '">';
                    // apply padding if required
                    $output .= '<input type="submit" name="page" value="'.($this->_properties['padding'] ? str_pad(($this->_properties['reverse'] ? 1 : $this->_properties['total_pages']), strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : ($this->_properties['reverse'] ? 1 : $this->_properties['total_pages'])).
    ...
    }private function _show_previous3()
    {
$idform = uniqid("form_");
        $output = '';
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            // if we're on the first page, the link is disabled
            $output = '<li' . ($this->_properties['page'] == 1 ? ' class="disabled"' : '') . '>';
            
            $output .= '<a onclick="';
            $output .= $this->_properties['page'] == 1 ? 'javascript:void(0);' : 'document.getElementById(\''.$idform.'\').submit();';
            $output .= '" rel="prev">';
                $output .= ($this->_properties['reverse'] ? $this->_properties['next'] : $this->_properties['previous']) . '</a>';
            $output .= '<form style="display:inline" method="post" id="'.$idform.'" action="'.($this->_properties['base_url']).'">';
            $output .= $this->repetir_post();
             $output .= '<input type="hidden" name="'.$this->_properties['variable_name'].'" value="'.($this->_properties['page'] == $this->_properties['total_pages'] ? 1 : $this->_properties['page'] - 1).'">';
            $output .= '</form>';
