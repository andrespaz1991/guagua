...
def connect_db():
    try:
        ...
        log_event("Conexión exitosa con el servidor MySQL.")
        return conn
    except mysql.connector.Error as err:
        log_event(f"Error de conexión: {err}")
        print("No se pudo conectar a la base de datos. Verifica el archivo hosting.log para más detalles.")
        return None
...
def get_latest_sql_file(directory):
    ...
    if not list_of_files:
        ...
        log_event("No se encontraron archivos .sql en el directorio.")
        return None
...
def import_database(sql_file):
    if not sql_file:
        ...
        log_event("No se encontró un archivo SQL para importar.")
        return
    
    if is_file_empty(sql_file):
        print(f"El archivo {sql_file} está vacío. No se puede importar.")
        log_event(f"El archivo {sql_file} está vacío. No se puede importar.")
        return
    ...
    log_event(f"Iniciando importación de {sql_file}. Tiempo estimado: {estimated_time} segundos.")
    ...
    try:
        ...
        log_event(f"Importación completada en {total_time:.2f} segundos desde {sql_file}.")
    except subprocess.CalledProcessError as e:
        print(f"Error al importar la base de datos: {e}")
        log_event(f"Error al importar la base de datos: {e}")
...
def main():
    ...
    if conn:
        ...
        log_event("Tablas eliminadas correctamente.")
        ...
        log_event("Ejecución del script omitida debido a falta de conexión.")
