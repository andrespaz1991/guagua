...
function normalizar_texto($string) {
    if (empty($string)) return $string;
    $string = (string)$string;
    // Fix non-UTF8 strings
    if (!mb_check_encoding($string, 'UTF-8')) {
        $string = mb_convert_encoding($string, 'UTF-8', 'Windows-1252');
    } else {
        // Fix double encoded UTF-8 strings (like DÃ‰CIMO -> DÉCIMO)
        $decoded = @utf8_decode($string);
        if ($decoded !== false && mb_check_encoding($decoded, 'UTF-8') && preg_match('/[\xC2-\xDF][\x80-\xBF]/', $decoded)) {
            $string = $decoded;
        }
    }
    return trim($string);
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if ($handle) {
            ...
            while (($data = fgetcsv($handle, 10000, $delimitador)) !== FALSE) {
                ...
                $grado_val = ($idx_grado !== false && isset($data[$idx_grado])) ? normalizar_texto($data[$idx_grado]) : '';
                $doc_val = ($idx_doc !== false && isset($data[$idx_doc])) ? normalizar_texto($data[$idx_doc]) : '';
                $tipo_doc_val = ($idx_tipo_doc !== false && isset($data[$idx_tipo_doc])) ? normalizar_texto($data[$idx_tipo_doc]) : '';
                $apellidos_val = ($idx_apellidos !== false && isset($data[$idx_apellidos])) ? normalizar_texto($data[$idx_apellidos]) : '';
                $nombres_val = ($idx_nombres !== false && isset($data[$idx_nombres])) ? normalizar_texto($data[$idx_nombres]) : '';
                $edad_val = ($idx_edad !== false && isset($data[$idx_edad])) ? normalizar_texto($data[$idx_edad]) : '';
                $etnia_val = ($idx_etnia !== false && isset($data[$idx_etnia])) ? normalizar_texto($data[$idx_etnia]) : '';
                $genero_val = ($idx_genero !== false && isset($data[$idx_genero])) ? normalizar_texto($data[$idx_genero]) : '';
                ...
                if ($doc_val !== '') {
                    ...
                    if ($tipo_doc_val === '') $tipo_doc_val = normalizar_texto(obtener_dato($doc_val, 'tipo_documento'));
                    if ($edad_val === '')     $edad_val     = normalizar_texto(obtener_dato($doc_val, 'fecha_nacimiento'));
                    if ($etnia_val === '')    $etnia_val    = normalizar_texto(obtener_dato($doc_val, 'etnia'));
                    if ($genero_val === '')   $genero_val   = normalizar_texto(obtener_dato($doc_val, 'genero'));
                }
                
                // Asegurar que el género esté en mayúsculas
                if ($genero_val !== '') {
                    $genero_val = mb_strtoupper($genero_val, 'UTF-8');
                }
...
<body class="bg-gray-100 min-h-screen p-8 font-sans">
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-6 border-b border-gray-200">
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
                <li class="mr-2">
                    <button onclick="switchMainTab('pae')" id="btn-main-pae" class="inline-block p-4 rounded-t-lg border border-transparent hover:text-gray-600 hover:border-gray-300 transition-colors tab-active">
                ...
                <li class="mr-2">
                    <button onclick="switchMainTab('fechas')" id="btn-main-fechas" class="inline-block p-4 rounded-t-lg border border-transparent hover:text-gray-600 hover:border-gray-300 transition-colors tab-inactive">
        ...
        <!-- Contenedor Pestaña Principal 1: Procesador PAE -->
        <div id="main-pae" class="block">
        ...
        <!-- Contenedor Pestaña Principal 2: Calculadora de Fechas -->
        <div id="main-fechas" class="hidden">
            <div class="bg-white rounded-lg shadow-lg p-6 border-t-4 border-teal-600">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Calculadora de Diferencia de Días</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Fecha Inicial</label>
                        <input type="date" id="fecha-inicio" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-teal-500 outline-none transition-shadow" onchange="calcularDiferenciaDias()">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Fecha Final</label>
                        <input type="date" id="fecha-fin" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-teal-500 outline-none transition-shadow" onchange="calcularDiferenciaDias()">
                    </div>
                </div>
                
                <div class="mt-8 p-6 bg-gray-50 rounded-lg border border-gray-200 text-center">
                    <p class="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">Resultado</p>
                    <p id="resultado-dias" class="text-4xl font-black text-teal-600">--</p>
                    <p id="texto-dias" class="text-gray-600 mt-2 font-medium">Seleccione ambas fechas para calcular</p>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function switchMainTab(tabId) {
            document.getElementById('main-pae').classList.add('hidden');
            document.getElementById('main-pae').classList.remove('block');
            document.getElementById('main-fechas').classList.add('hidden');
            document.getElementById('main-fechas').classList.remove('block');
            
            document.getElementById('btn-main-pae').classList.remove('tab-active', 'border-b-0');
            document.getElementById('btn-main-pae').classList.add('tab-inactive');
            document.getElementById('btn-main-fechas').classList.remove('tab-active', 'border-b-0');
            document.getElementById('btn-main-fechas').classList.add('tab-inactive');
            
            document.getElementById('main-' + tabId).classList.remove('hidden');
            document.getElementById('main-' + tabId).classList.add('block');
            
            const activeBtn = document.getElementById('btn-main-' + tabId);
            activeBtn.classList.remove('tab-inactive');
            activeBtn.classList.add('tab-active', 'border-b-0');
        }

        // Lógica para calcular diferencia asincrona y dinámica
        function calcularDiferenciaDias() {
            ...
            const resultadoEl = document.getElementById('resultado-dias');
            const textoEl = document.getElementById('texto-dias');

            if (!inicioVal || !finVal) {
                resultadoEl.innerText = '--';
                textoEl.innerText = 'Seleccione ambas fechas para calcular';
                return;
            }

            const fecha1 = new Date(inicioVal);
            const fecha2 = new Date(finVal);
            
            // Calculo dinamico nativo de Javascript (Asíncrono respecto al servidor)
            const diferenciaTiempo = fecha2.getTime() - fecha1.getTime();
            const diferenciaDias = Math.ceil(diferenciaTiempo / (1000 * 3600 * 24));
            
            resultadoEl.innerText = Math.abs(diferenciaDias) + ' días';
            
            if (diferenciaDias > 0) {
                textoEl.innerText = 'La fecha final es posterior a la fecha inicial';
                resultadoEl.className = 'text-4xl font-black text-teal-600';
            } else if (diferenciaDias < 0) {
                textoEl.innerText = 'La fecha final es anterior a la fecha inicial';
                resultadoEl.className = 'text-4xl font-black text-rose-600';
            } else {
                textoEl.innerText = 'Ambas fechas son exactamente iguales';
                resultadoEl.className = 'text-4xl font-black text-slate-600';
            }
        }
